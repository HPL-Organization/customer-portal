...
export async function getEventTypeByInternalName(internalName: string): Promise<LiveEventType | undefined> {
...
export async function joinLiveSession(eventId: string, args: JoinLiveSessionArgs) {
  try {
    const token = process.env.LIVESALEAPP_TOKEN;
    ...
    if (joinUrlData.code === "ZOOM_JOIN_URL_NOT_FOUND") {
      ...
      if (!createJoinResponse.ok) {
        throw new Error(`Failed to create zoom join URL: ${createJoinResponse.status}`);
      }
