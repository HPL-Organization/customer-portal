// app/api/netsuite/event-types/route.ts
...
export async function GET(req: NextRequest) {
  const recordType =
    req.nextUrl.searchParams.get("rt") || "customlist_hpl_eventslist";
  ...
  if (!r.ok) {
    return NextResponse.json(
      { error: "Failed to list event types", status: r.status, details: text },
