...
export function PromotionsView({ prefs, setPrefs, onBack }: { 
  prefs: any, 
  setPrefs: (p: any) => void, 
  onBack: () => void 
}) {
  ...
  const sections = [
    { id: 'general', title: 'General', value: prefs.general },
    { id: 'discounts', title: 'Discounts and Giveaway', value: prefs.discounts },
    { id: 'newProducts', title: 'New machines, rough rock, or finished goods', value: prefs.newProducts },
  ];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Promotions preferences saved!");
      onBack();
    } catch (err: any) {
      console.error("[comm-prefs:promotions]", err);
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-6">
      <TitleSection 
        ...
        redSubtitle="Select whether to receive emails on any updates. Choose the frequency."
      />
      
      <div className="bg-[#e5e7eb] rounded-xl p-8 shadow-inner">
        <h2 className="text-xl font-bold text-zinc-900 mb-6">Promotions and Announcements</h2>
        
        {sections.map((section, sIdx) => (
          <div key={section.id} className={`bg-[#d1d5db]/60 rounded-xl overflow-hidden border border-zinc-300 ${sIdx !== sections.length - 1 ? 'mb-6' : ''}`}>
            ...
            {options.map((opt, idx) => (
              <div key={opt.id} className={`flex justify-between items-center py-4 px-6 ${idx !== options.length - 1 ? 'border-b border-zinc-300' : ''}`}>
                ...
                <Checkbox 
                  checked={section.value === opt.id} 
                  onChange={() => setPrefs({ ...prefs, [section.id]: opt.id })} 
                />
              </div>
            ))}
          </div>
